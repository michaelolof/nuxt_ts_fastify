...
const fastifyAutoload = async function autoload(fastify: FastifyInstance, options: any) {
    ...
    function replaceRouteParamPattern(pattern: string) {
...
async function findPlugins(dir: string, options: Record<string, any>, hookedAccumulator = {}, prefix?: string, depth = 0, hooks = []) {
...
async function loadPlugin({ file, type, directoryPrefix, options, log }) {
    ...
    if (forceESM || type === "module" || runtime.forceESM) {
        content = await tsImport(basename(file), pathToFileURL(file).href)
